...
func (a DouyinAdapter) GetShortVideoInfo(url string) (*ShortVideoInfoResponse, error) {
	...
	getResp, err := restyHttp.GetMobileHttpRequest().Get("https://www.iesdouyin.com/aweme/v1/web/aweme/detail/?aweme_id=" + match[1])
	if err != nil {
		return nil, err
	}
	...
	shortVideoInfo := &ShortVideoInfoResponse{
		AuthorName:             douyinResponse.AwemeDetail.Author.Nickname,
		AuthorAvatar:           douyinResponse.AwemeDetail.Author.AvatarThumb.URLList[0],
		Title:                  douyinResponse.AwemeDetail.Desc,
		Cover:                  douyinResponse.AwemeDetail.Video.OriginCover.URLList[0],
		CreatedAt:              "",
		NoWatermarkDownloadUrl: douyinResponse.AwemeDetail.Video.PlayAddr.URLList[0],
	}
